...
def _parse_date(string):
    ...
    try:
        ...
        print("dateparser is not installed. to convert strings to dates" "install via `pip install dateparser`.")
        raise ValueError("birthdate must be datetime object or of format" " `%d-%m-%Y`, eg. `24-01-2020`")
...
def read_edf(edf_file, ch_nrs=None, ch_names=None, digital=False, verbose=False):
    ...
    with pyedflib.EdfReader(edf_file) as f:
        ...
        signals = []
        for i, c in enumerate(tqdm(ch_nrs, desc="Reading Channels", disable=not verbose, leave=False)):
...
def write_edf(edf_file, signals, signal_headers, header=None, digital=False, file_type=-1, block_size=1):
    """
    ...
    assert block_size <= 60 and block_size >= -1 and block_size != 0, "blocksize must be smaller or equal to 60"
    ...
    if block_size == -1:
        ...
        warnings.warn("Signal length is not dividable by block_size. " + "The file will have a zeros appended.")
    ...
    for sig, shead in zip(signals, signal_headers):
        ...
        if digital:  # exception as it will lead to clipping
            assert dmin <= sig.min(), "digital_min is {}, but signal_min is {}" "for channel {}".format(
                dmin, sig.min(), label
            )
            assert dmax >= sig.max(), "digital_min is {}, but signal_min is {}" "for channel {}".format(
                dmax, sig.max(), label
            )
            assert pmin != pmax, "physical_min {} should be different from physical_max {}".format(pmin, pmax)
        else:  # only warning, as this will not lead to clipping
            assert pmin <= sig.min(), "phys_min is {}, but signal_min is {} " "for channel {}".format(
                pmin, sig.min(), label
            )
            assert pmax >= sig.max(), "phys_max is {}, but signal_max is {} " "for channel {}".format(
                pmax, sig.max(), label
            )
...
def write_edf_quick(edf_file, signals, sfreq, digital=False):
    ...
    signal_headers = make_signal_headers(labels, sample_rate=sfreq, physical_min=pmin, physical_max=pmax)
...
def compare_edf(edf_file1, edf_file2, verbose=False):
    ...
    for i, sigs in enumerate(zip(signals1, signals2)):
        ...
        assert close > 0.99, "Error, digital values of {}" " and {} for ch {}: {} are not the same: {:.3f}".format(
    ...
    for i, sigs in enumerate(zip(signals1, signals2)):
        ...
        assert close > 0.99, "Error, physical values of {}" " and {} for ch {}: {} are not the same: {:.3f}".format(
            edf_file1, edf_file2, shead1[i]["label"], shead2[i]["label"], close
        )
    return True


def drop_channels(edf_source, edf_target=None, to_keep=None, to_drop=None, verbose=False):
    """
    ...
    assert os.path.exists(edf_source), "source file {} does not exist".format(edf_source)
    ...
    signals, signal_headers, header = read_edf(edf_source, ch_nrs=load_channels, digital=True, verbose=verbose)
...
def anonymize_edf(
    edf_file, new_file=None, to_remove=["patientname", "birthdate"], new_values=["xxx", ""], verify=False, verbose=False
):
...
def rename_channels(edf_file, mapping, new_file=None, verbose=False):
    ...
    for ch_nr in tqdm(range(len(channels)), disable=not verbose):
        signal, signal_header, _ = read_edf(file, digital=True, ch_nrs=ch_nr, verbose=verbose)
...
def change_polarity(edf_file, channels, new_file=None, verify=True, verbose=False):
    """
    ...
    for i, sig in enumerate(signals):
        ...
        if label in channels:
            ...
            signals[i] = -sig
    write_edf(new_file, signals, signal_headers, header, digital=True, correct=False, verbose=verbose)
    if verify:
